import { Plus, FileText, Radio, Scale, Database, PlusCircle, Upload, BarChart3 } from "lucide-react"
...
const statCards = [
  { icon: FileText, label: "Total Exams", value: "0", subtitle: "No exams recorded" },
  { icon: Radio, label: "Active Now", value: "0", subtitle: "Idle", iconColorClassName: "text-secondary" },
  { icon: Scale, label: "Flagged Today", value: "0", subtitle: "Clean slate", iconColorClassName: "text-error" },
  { icon: Database, label: "Question Bank", value: "0", subtitle: "Library empty", iconColorClassName: "text-tertiary" },
]
...
export default function ProfessorDashboard() {
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      >
        <Button asChild className="bg-primary-container text-on-primary-container hover:brightness-110 shadow-lg shadow-primary/10 sm:px-6 sm:py-3 sm:h-auto sm:rounded-lg sm:gap-2 w-12 h-12 rounded-xl sm:w-auto">
          <Link to={PATHS.PROFESSOR.EXAM_BUILDER}>
            <Plus size={20} />
            <span className="hidden sm:inline font-semibold text-sm">Start New Session</span>
          </Link>
        </Button>
      </SectionHeader>
